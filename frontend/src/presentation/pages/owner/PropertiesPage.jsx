...
function emptyForm() {
  return { title: "", description: "", address: "", price: "", bedrooms: "", bathrooms: "", is_furnished: false, property_type_id: "", listing_type_id: "", district_id: "" };
}
...
export default function PropertiesPage() {
  ...
  useEffect(() => {
    ...
    ]).then(([d, pt, lt]) => { setDistricts(d.data); setPropTypes(pt.data); setListTypes(lt.data); });
    load();
  }, []);

  const openCreate = () => { setForm(emptyForm()); setModal({ type: "create" }); };
  const openEdit = (p) => {
    setForm({ title: p.title, description: p.description ?? "", address: p.address, price: p.price ?? "", bedrooms: p.bedrooms ?? "", bathrooms: p.bathrooms ?? "", is_furnished: p.is_furnished ?? false, property_type_id: p.property_type_id, listing_type_id: p.listing_type_id, district_id: p.district_id ?? "" });
  ...
  const save = async () => {
    const payload = { ...form, price: form.price ? Number(form.price) : null, bedrooms: form.bedrooms ? Number(form.bedrooms) : null, bathrooms: form.bathrooms ? Number(form.bathrooms) : null, property_type_id: Number(form.property_type_id), listing_type_id: Number(form.listing_type_id), district_id: form.district_id ? Number(form.district_id) : null };
    try {
      ...
      {loading ? <p className="text-gray-400">Loading…</p> : (
        <div className="card p-0 overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead>
              <tr>{["Title", "Address", "Type", "Price", "Status", "Actions"].map((h) => <th key={h} className="table-header px-4 py-3">{h}</th>)}</tr>
            </thead>
